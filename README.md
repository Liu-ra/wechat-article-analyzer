...
2. 输入任意一篇该公众号的文章链接
3. 点击「开始批量分析」
4. **推荐：**点击「一键自动获取 Cookie」按钮
   - 在弹出窗口中使用微信扫码登录
   - 系统自动提取并验证 Cookie
5. **或者：**手动粘贴 Cookie（如果自动获取失败）
   - 复制生成的公众号历史消息页面链接
   - 在微信中打开该链接
   - 使用抓包工具获取Cookie（如Fiddler、Charles）
   - 将Cookie粘贴到应用中
6. 设置要获取的文章数量（建议不超过100篇）
7. 点击「开始获取文章」
8. 导出Excel报告
...
#### 方法一：一键自动获取（强烈推荐）⭐
1. 在批量分析步骤中，点击「一键自动获取 Cookie」按钮
2. 应用会自动打开微信登录窗口
3. 使用微信扫描二维码登录
4. 登录成功后，窗口自动关闭，Cookie 自动提取并验证
5. 无需任何手动操作，一键完成
